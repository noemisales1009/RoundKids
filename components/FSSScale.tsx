...
export const FSSScale: React.FC<ScaleProps> = ({ onSaveScore }) => {
  const [score, setScore] = useState(0);

  const handleSave = () => {
    onSaveScore({ scaleName: 'FSS', score, interpretation: 'Avaliado' });
  };

  return (
    <div className="space-y-4 bg-slate-900 p-6 rounded-lg">
      <h3 className="text-xl font-bold text-slate-100">Escala de Status Funcional (FSS)</h3>
