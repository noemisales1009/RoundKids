...
const getInterpretacaoFSS = (total: number) => {
  ...
  if (total <= 9) {
    ...
    return { 
      ...
      bgCor: 'bg-primary-900/30',
      borderCor: 'border-primary-500/50',
...
const DropdownFSS = forwardRef<HTMLDivElement, DropdownFSSProps>(({ label, id, valor, onOpcaoChange, opcoes }, ref) => (
  <div ref={ref} className="bg-linear-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-700 p-4 rounded-xl shadow-lg transition-all duration-300 border border-slate-200 dark:border-slate-600 hover:border-primary-500/50">
    <div className="flex justify-between items-center mb-3">
      <label
        htmlFor={id}
        className="block text-sm font-semibold text-slate-600 dark:text-slate-200"
      >
        {label}
      </label>
    ...
    <select
      ...
      className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-500 text-slate-900 dark:text-slate-100 p-3 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 focus:outline-none transition-colors hover:border-slate-400"
    >
      <option value="" className="text-slate-500 dark:text-slate-400">Selecione uma opção...</option>
      {opcoes.map((opt) => (
        <option key={opt.texto} value={opt.valor} className="bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100">
          {opt.texto}
        </option>
      ))}
    </select>
      ...
      {telaAtiva === 'lista' && (
        <div className="flex flex-col space-y-4">
          <div className="bg-linear-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-700 p-6 rounded-xl shadow-lg text-center border border-slate-200 dark:border-slate-600">
            <h2 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-4 letter-spacing-1">
            ...
            <div className="flex items-baseline justify-center space-x-1 mb-6">
              ...
              <span className="text-slate-500 dark:text-slate-400 text-xl">/30</span>
            </div>
          ...
          <button
            ...
            className="w-full bg-linear-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-bold py-3 px-4 rounded-lg shadow-md transition-all duration-200 transform hover:scale-105 active:scale-95"
          >
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex items-center space-x-4 mb-2 pb-4 border-b border-slate-200 dark:border-slate-600">
            <button
              ...
              className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
            >
              <IconeVoltar />
            </button>
          ...
          <div className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-600 mt-2">
            <div className="flex justify-between items-center mb-2">
              <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">Progresso</span>
            ...
            <div className="w-full bg-slate-200 dark:bg-slate-700 h-2 rounded-full overflow-hidden">
              <div 
                className="bg-linear-to-r from-primary-500 to-primary-600 h-full transition-all duration-300"
          ...
          <button
            ...
            className="w-full bg-linear-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-bold py-3 px-4 rounded-lg shadow-md transition-all duration-200 transform hover:scale-105 active:scale-95 mt-4"
          >
            Calcular Pontuação
          </button>
        </div>
      ...
      {telaAtiva === 'resultado' && (
        <div className="flex flex-col space-y-6">
          ...
          <div className="text-center my-6">
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-2">Pontuação Total</p>
            <div className="flex items-baseline justify-center space-x-2">
              <span className="text-8xl font-black text-slate-900 dark:text-white">
                {pontuacaoTotalCalculada}
              </span>
              <span className="text-3xl text-slate-400 dark:text-slate-500">/30</span>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-800 p-4 rounded-lg border border-slate-200 dark:border-slate-600 space-y-2">
            <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase">Recomendações:</h3>
          ...
          <button
            ...
            className="w-full bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 font-semibold py-3 rounded-lg transition-colors border border-slate-200 dark:border-slate-600"
          >
