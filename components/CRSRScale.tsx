...
export const CRSRScale: React.FC<ScaleProps> = ({ onSaveScore }) => {
  const [score, setScore] = useState(0);

  const handleSave = () => {
    onSaveScore({ scaleName: 'CRS-R', score, interpretation: 'Avaliado' });
  };

  return (
    <div className="space-y-4 bg-slate-900 p-6 rounded-lg">
      <h3 className="text-xl font-bold text-slate-100">Escala de Recuperação de Coma (CRS-R)</h3>
