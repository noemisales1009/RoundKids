...
interface ArchiveAporteModalProps {
    aporteId: string;
    aporteDate: string;
    onClose: () => void;
    onSuccess: () => void;
...
export const ArchiveAporteModal: React.FC<ArchiveAporteModalProps> = ({
    aporteId,
    aporteDate,
    onClose,
    onSuccess,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showNotification({ message: 'Aporte arquivado com sucesso!', type: 'success' });
            onSuccess();
            onClose();
        } catch (error) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-30" role="dialog" aria-modal="true" aria-label="Arquivar Aporte">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-md m-4">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Arquivar Aporte</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                    <p className="text-sm text-amber-800 dark:text-amber-200 font-medium">
                        Aporte do dia {new Date(aporteDate).toLocaleDateString('pt-BR')}
                    </p>
                </div>
