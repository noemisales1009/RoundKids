...
export const AddAporteModal: React.FC<{
  ...
  const [dataReferencia, setDataReferencia] = useState(getTodayDateString());
  const [vo, setVo] = useState('0');
  const [hvNpt, setHvNpt] = useState('0');
  const [medicacoes, setMedicacoes] = useState('0');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification({ message: 'Aporte cadastrado com sucesso!', type: 'success' });
      onSuccess();
      onClose();
    } catch (error: any) {
      console.error('Erro ao cadastrar aporte:', error);
      showNotification({ 
        message: error?.message || 'Erro ao cadastrar aporte', 
        type: 'error' 
      });
    } finally {
      setLoading(false);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-md m-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Novo Aporte</h2>
