...
export const AlertModal: React.FC<{ question: Question, onClose: () => void, patientId: string }> = ({ question, onClose, patientId }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-slate-800 dark:bg-slate-900 rounded-xl w-full max-w-md shadow-2xl border border-primary-500/30 flex flex-col max-h-[95vh]">

                {/* Cabeçalho */}
                <div className="bg-primary-600 p-4 rounded-t-xl flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-2">
                        <AlertIcon className="text-white w-6 h-6" />
                        <h2 className="text-white font-bold text-lg">Alerta / Intervenção</h2>
                    </div>
                    <button onClick={onClose} className="text-white/80 hover:text-white bg-primary-700/50 p-1.5 rounded-full hover:bg-primary-700 transition">
                ...
                <div className="flex-1 overflow-y-auto p-5 custom-scrollbar space-y-6">
                    <div className="border-b border-slate-700 pb-2">
                        <h3 className="text-primary-300 font-bold text-sm uppercase tracking-wide">
                            {question.text}
                        </h3>
                        <p className="text-slate-400 text-xs mt-1">Selecione as intercorrências:</p>
                    </div>
                            ...
                            return (
                                <div key={opt.id} className="flex flex-col gap-2">
                                    <div
                                        ...
                                        className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition border ${isChecked ? 'bg-primary-900/20 border-primary-500/30' : 'bg-slate-900 border-slate-700 hover:border-slate-500'}`}
                                    >
                                        <div className={isChecked ? 'text-primary-400' : 'text-slate-500'}>
                                            {isChecked ? <CheckSquareIcon className="w-5 h-5" /> : <SquareIcon className="w-5 h-5" />}
                                        </div>
                                        <span className={`text-sm ${isChecked ? 'text-white font-medium' : 'text-slate-300'}`}>{opt.label}</span>
                                    </div>
                                    {isChecked && opt.hasInput && (
                                        <div className="ml-9 animate-in slide-in-from-top-2 duration-200">
                                            <input
                                                ...
                                                className="w-full bg-slate-900 border border-primary-500/50 rounded p-3 text-white text-sm focus:outline-none focus:ring-1 focus:ring-primary-500"
                                                autoFocus
                                            />
                                        </div>
                                    )}
                    ...
                    <div className="pt-4 border-t border-slate-700 space-y-4 bg-slate-800 dark:bg-slate-900">
                        {/* Responsável */}
                        <div>
                            <label className="text-white font-semibold text-sm mb-1 block">Responsável</label>
                            <div className="relative">
                                <select value={responsible} onChange={(e) => setResponsible(e.target.value)} className="w-full bg-slate-700 border border-slate-600 text-white text-sm rounded-lg p-3 appearance-none focus:outline-none">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select value={time} onChange={(e) => setTime(e.target.value)} className="w-full bg-slate-700 border border-slate-600 text-white text-sm rounded-lg p-3 appearance-none focus:outline-none">
                                    <option value="">Selecione...</option>
                        ...
                        <button onClick={handleSave} disabled={isLoading} className="w-full mt-2 flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-500 text-white py-3 rounded-lg font-bold shadow-lg transition">
