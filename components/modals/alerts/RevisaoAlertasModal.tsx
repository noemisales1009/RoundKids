...
export const RevisaoAlertasModal: React.FC<RevisaoAlertasModalProps> = ({
    pendentes,
    onClose,
    onLiberado,
    onAlertaTratado,
    ...
    const visiveis = pendentes.filter(a => !removidos.includes(a.id));
    const faltam = visiveis.filter(a => !justificados[a.id]);
    ...
    const handleConcluir = async (alerta: Alerta) => {
        ...
        if (ok) {
            ...
            fecharAcao();
            onAlertaTratado();
            showNotification({ message: 'Alerta concluído!', type: 'success' });
        } else {
            showNotification({ message: 'Erro ao concluir alerta', type: 'error' });
        }
    };
    ...
    const handleArquivar = async (alerta: Alerta) => {
        ...
        if (!motivo) {
            showNotification({ message: 'Informe o motivo do arquivamento', type: 'error' });
            return;
        }
        if (!exigeUsuario()) return;
        setSalvandoId(alerta.id);
        const ok = await alertasService.arquivarAlerta(alerta.id, motivo, alerta.source, user!.id!);
        setSalvandoId(null);
        if (ok) {
            setRemovidos(prev => [...prev, alerta.id]);
            fecharAcao();
            onAlertaTratado();
            showNotification({ message: 'Alerta arquivado!', type: 'success' });
        } else {
    ...
    const abrirArquivamento = (alerta: Alerta) => {
        setAcaoAberta({ id: alerta.id, modo: 'arquivar' });
        setTexto('');
    };
                        ...
                        visiveis.map((alerta) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {/* Justificativa herdada de turno anterior */}
