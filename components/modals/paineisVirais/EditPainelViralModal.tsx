...
export type PainelViralRow = {
    ...
    diagnostico_label?: string;
    diagnostico_data_inicio?: string;
};
...
export const EditPainelViralModal: React.FC<{
    ...
    const [sistemaOutros, setSistemaOutros] = useState(
        painelData.sistema && !ALERT_SYSTEMS.includes(painelData.sistema) ? painelData.sistema : ''
    );
    ...
    const [selectedDiagnosticoId, setSelectedDiagnosticoId] = useState<number | ''>(painelData.diagnostico_id ?? '');
    ...
    useEffect(() => {
        const fetchDiagnosticos = async () => {
            const { data } = await supabase
                ...
                .eq('patient_id', painelData.paciente_id)
                .eq('arquivado', false)
