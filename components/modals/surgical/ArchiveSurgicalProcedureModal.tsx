...
interface ArchiveSurgicalProcedureModalProps {
...
export const ArchiveSurgicalProcedureModal: React.FC<ArchiveSurgicalProcedureModalProps> = ({ procedure, patientId, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) {
                ...
                showNotification({ message: 'Erro ao arquivar procedimento', type: 'error' });
                return;
            }

            showNotification({ message: 'Procedimento cirúrgico arquivado com sucesso!', type: 'success' });
            onSuccess();
            onClose();
        } catch (error) {
            console.error('Erro ao arquivar procedimento:', error);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-30">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-md m-4">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Arquivar Procedimento Cirúrgico</h2>
                    <button 
                        onClick={onClose} 
                ...
                <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                    ...
                    {procedure.date && (
                        <p className="text-xs text-amber-700 dark:text-amber-300 mt-1">
                            Data: {new Date(procedure.date).toLocaleDateString('pt-BR')}
                        </p>
                    )}
                    {procedure.surgeon && (
                        <p className="text-xs text-amber-700 dark:text-amber-300">
                            Cirurgião: {procedure.surgeon}
                        </p>
                    )}
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Exemplo: Procedimento cancelado, registro duplicado, etc.
                        </p>
                    </div>
