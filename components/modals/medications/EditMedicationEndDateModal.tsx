...
export const EditMedicationEndDateModal: React.FC<{ medication: Medication; patientId: number | string; onClose: () => void; }> = ({ medication, patientId, onClose }) => {
    ...
    const handleRevertEnd = () => {
        updateMedicationInPatient(patientId, { ...medication, endDate: undefined });
        showNotification({ message: 'Data de fim revertida com sucesso! A medicação voltou como ativa.', type: 'success' });
        onClose();
    };
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-sm m-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="mt-1 block w-full border border-slate-300 dark:border-slate-700 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200" required />
                    </div>
                    <button
                        type="button"
