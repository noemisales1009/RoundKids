...
interface ArchiveMedicationModalProps {
...
export const ArchiveMedicationModal: React.FC<ArchiveMedicationModalProps> = ({ medication, patientId, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) {
                console.error('Erro ao arquivar medicação:', error);
                showNotification({ message: 'Erro ao arquivar medicação', type: 'error' });
                return;
            }

            showNotification({ message: 'Medicação arquivada com sucesso!', type: 'success' });
            onSuccess();
            onClose();
        } catch (error) {
            console.error('Erro ao arquivar medicação:', error);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-30">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-md m-4">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Arquivar Medicação</h2>
                    <button 
                        onClick={onClose} 
                ...
                <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                    ...
                    {medication.endDate && (
                        <p className="text-xs text-amber-700 dark:text-amber-300">
                            Fim: {new Date(medication.endDate).toLocaleDateString('pt-BR')}
                        </p>
                    )}
                </div>
