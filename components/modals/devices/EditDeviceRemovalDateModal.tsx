...
export const EditDeviceRemovalDateModal: React.FC<{ device: Device; patientId: number | string; onClose: () => void; }> = ({ device, patientId, onClose }) => {
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow-xl w-full max-w-sm m-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="date" value={removalDate} onChange={e => setRemovalDate(e.target.value)} className="mt-1 block w-full border border-slate-300 dark:border-slate-700 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200" required />
                    </div>
                    <button
                        type="button"
