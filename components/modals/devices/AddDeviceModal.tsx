...
export const AddDeviceModal: React.FC<{ patientId: number | string; onClose: () => void; }> = ({ patientId, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const result = await addDeviceToPatient(patientId, { name: finalType, location, startDate, observacao, sistema: (sistema === 'Outros' ? sistemaOutros.trim() : sistema) || undefined }, user.id);
