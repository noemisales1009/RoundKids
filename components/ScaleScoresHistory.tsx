...
const getScaleColor = (scaleName: string) => {
  ...
  if (scaleName.includes('FSS')) return { bg: 'bg-green-500/10', border: 'border-green-500', text: 'text-green-600 dark:text-green-400' };
  return { bg: 'bg-slate-500/10', border: 'border-slate-500', text: 'text-slate-500 dark:text-slate-400' };
};
...
export default function ScaleScoresHistory({ patientId }: ScaleScoresHistoryProps) {
  ...
  if (isLoading) {
    return (
      <div className="p-6 rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-center">
        <div className="animate-pulse text-slate-500 dark:text-slate-400">Carregando histórico...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 rounded-2xl border-2 border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 text-sm">
        {error}
      </div>
    );
  }
  ...
  if (!scores || scores.length === 0) {
    return (
      <div className="p-6 rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
        <p className="text-slate-500 dark:text-slate-400 text-sm text-center">Nenhuma escala avaliada ainda</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider px-2">
        ...
        {scores.map((score) => {
          ...
          return (
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="flex items-center gap-2">
                      <span className="text-2xl font-bold text-slate-900 dark:text-white">{score.score}</span>
              ...
              {isExpanded && (
                <div className="px-4 pb-4 border-t-2 border-slate-200 dark:border-slate-700 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="space-y-3 pt-4">
                    <div>
                      <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Escala</p>
                      <p className="text-sm text-slate-700 dark:text-slate-200">{score.scale_name}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Pontuação</p>
                        <p className={`text-2xl font-bold ${colors.text}`}>{score.score}</p>
                      </div>
                      <div>
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Resultado</p>
                        <p className={`text-sm font-semibold ${interpretationColor}`}>{score.interpretation}</p>
                      </div>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Data e Hora</p>
                      <p className="text-sm text-slate-600 dark:text-slate-300">{formattedDate} às {formattedTime}</p>
                    </div>
                    <div className="pt-2 border-t border-slate-200 dark:border-slate-700">
                      <p className="text-xs text-slate-500 dark:text-slate-400">ID: {score.id}</p>
                    </div>
                  </div>
                </div>
              )}
