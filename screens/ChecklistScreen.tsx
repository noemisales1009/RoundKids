...
export const ChecklistScreen: React.FC = () => {
    ...
    return (
        <div className="relative min-h-screen pb-6 px-4 flex items-center justify-center">
            {/* Main Card */}
            <div className="w-full max-w-lg bg-primary-600 dark:bg-primary-700 rounded-xl shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-right-4 duration-300">

                {/* Content */}
                <div className="p-6 sm:p-8 flex-1 flex flex-col items-center text-center space-y-4 sm:space-y-6">
                    <span className="bg-primary-800/50 text-primary-100 px-4 py-1.5 rounded-full text-xs font-bold tracking-wider uppercase">
                        ...
                        {(['sim', 'não', 'nao_se_aplica'] as Answer[]).map(answer => (
                            <button
                                ...
                                ${currentAnswer === answer
                                        ? 'bg-primary-800 text-white border-white ring-2 ring-primary-400'
                                        : 'bg-primary-500 hover:bg-primary-400 text-white border-primary-400/30'}`}
                            >
                    ...
                    <div className="h-4 flex items-center justify-center">
                        {savingAnswer && (
                            <span className="flex items-center gap-2 text-primary-100 text-xs font-semibold">
                                <span className="inline-block w-3 h-3 border-2 border-primary-200 border-t-transparent rounded-full animate-spin" />
                                Salvando…
                            </span>
                        )}
                    </div>
                ...
                {/* Footer Navigation */}
                <div className="p-4 sm:p-6 border-t border-primary-500/30 flex justify-between items-center bg-primary-700/20">
                    <button
                        onClick={handlePrevious} disabled={currentQuestionIndex === 0}
                        className={`px-4 sm:px-6 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition flex items-center gap-2 ${currentQuestionIndex === 0 ? 'opacity-50 cursor-not-allowed text-primary-300' : 'bg-primary-800/50 hover:bg-primary-800 text-primary-100 hover:text-white'}`}
                    >
                    ...
                    <button
                        onClick={handleNext}
                        className={`px-6 sm:px-8 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition shadow-lg flex items-center gap-2 bg-white hover:bg-slate-100 text-primary-700`}
                    >
