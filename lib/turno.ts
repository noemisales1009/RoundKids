export type Turno = 'manha' | 'tarde' | 'noite';

// Turno atual no horário de Brasília: manhã 7h-13h, tarde 13h-19h, noite 19h-7h
export const turnoAtualSP = (ref: Date = new Date()): Turno => {
    const h = new Date(ref.getTime() - 3 * 60 * 60 * 1000).getUTCHours();
    if (h >= 7 && h < 13) return 'manha';
    if (h >= 13 && h < 19) return 'tarde';
    return 'noite';
};

export const LABEL_TURNO: Record<Turno, string> = { manha: 'Manhã', tarde: 'Tarde', noite: 'Noite' };

const ORDEM_TURNOS: Turno[] = ['manha', 'tarde', 'noite'];

// Turnos que aceitam um registro feito agora: o turno atual e os que já passaram no mesmo
// dia da evolução. Usado no alerta lançado com atraso (quem está na tarde e esqueceu de
// registrar de manhã). Na manhã só existe o próprio turno.
export const turnosParaRegistro = (ref: Date = new Date()): Turno[] =>
    ORDEM_TURNOS.slice(0, ORDEM_TURNOS.indexOf(turnoAtualSP(ref)) + 1);

// Turno e "dia da evolução" de um instante (ISO), em Brasília. A noite antes das 7h pertence ao dia anterior.
export const turnoEDiaDe = (iso: string): { turno: Turno; dia: string } => {
    const sp = new Date(new Date(iso).getTime() - 3 * 60 * 60 * 1000);
    const h = sp.getUTCHours();
    const turno: Turno = h >= 7 && h < 13 ? 'manha' : h >= 13 && h < 19 ? 'tarde' : 'noite';
    const ref = new Date(sp.getTime() - (h < 7 ? 24 * 60 * 60 * 1000 : 0));
    return { turno, dia: ref.toISOString().split('T')[0] };
};

// Turno de um registro (Diurese/BH): o escolhido no cadastro; registros antigos usam a hora em que foram salvos.
export const turnoDoRegistro = (r: { created_at: string; turno?: string | null }): Turno =>
    r.turno === 'manha' || r.turno === 'tarde' || r.turno === 'noite' ? r.turno : turnoEDiaDe(r.created_at).turno;

// Momento (ISO) para gravar um registro de um dia/turno escolhido no cadastro.
// Se o dia é o de hoje, vale a hora real; se é um dia passado (evolução esquecida), usa um horário dentro do turno.
export const instanteDoRegistro = (dia: string, turno: Turno): string => {
    if (dia === turnoEDiaDe(new Date().toISOString()).dia) return new Date().toISOString();
    const hora = { manha: 10, tarde: 16, noite: 22 }[turno];
    const [a, m, d] = dia.split('-').map(Number);
    return new Date(Date.UTC(a, m - 1, d, hora + 3, 0, 0)).toISOString();
};
