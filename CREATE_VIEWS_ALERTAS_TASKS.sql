-- =============================================
-- VIEW COMPLETA DE ALERTAS COM NOME DO PACIENTE
-- =============================================
...
base AS (
  SELECT
    ...
    t.patient_id,
    t.alerta_descricao,
    COALESCE(ur.name, t.responsavel, 'Não informado'::text) AS responsavel,
    t.hora_selecionada,
    t.status,
    t.justificativa,
    t.created_at,
  ...
  FROM
    ...
    LEFT JOIN users ur ON t.responsavel = ur.id::text OR t.responsavel = ur.name
)
SELECT
  id AS id_alerta,
  TO_CHAR(
...
-- =============================================
-- VIEW DE TASKS COM HORÁRIO BR E NOME PACIENTE
-- =============================================
